...
* Guix OS Configuration
  ...
  and staged as a distribution utilizing the new feature =guix-home-service-type= and
  specifying channels therein under =guix-configuration=. All that is needed is to run
  =sudo guix system reconfigure guix-config.scm= and everything will be set up after
  initial Guix SD (nonguix) installation!

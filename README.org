...
** Initialization File:  =~/.config/stumpwm/config= -> Loads in modules and set's up core
features, such amy X11 environment.
...
- =commands= - Commands, Helper Functions, and Macros for StumpWM (wip)
...
- =utilities= - Utility packages/libraries and their customization.

** Libraries:
- =audio-wpctl= - converting to package/library to push upstream

...
* Guix OS Configuration
  ...
  Guix OS is now fully deployed via Guix/Guile using =home-impure-symlinks-service-type=
  and staged as a distribution utilizing the new feature =guix-home-service-type=. All that is needed is to run =sudo guix system reconfigure guix-config.scm= and everything will be

...
** Home Configuration Outline (=home.scm=)
 ...
 - 1. =%logoraz-packages= -> My everyday packages
      (i.e. fonts, web, apps, tools)
 - 2. =%x11-util-packages= -> X11 packages needed for StumpWM
 - 3. =%emacs-packages= -> Use Guix to setup Emacs (it's all there!)
 - 4. =services= -> wip (todo: modularize to separate variable definitions)
